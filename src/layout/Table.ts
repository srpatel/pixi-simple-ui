...
export default class Table extends SizeableContainer {
  ...
  makeElement(
    e: PIXI.Container,
    strategy: SizingStrategy | PIXI.Point = null,
  ...
  update(forceUpdateChildren = false) {
    ...
    for (let i = 0; i < this.rows.length; i++) {
      ...
      for (let j = 0; j < row.cells.length; j++) {
        ...
        if (this.debug) {
          this.debugGraphic
            .rect(currentX, currentY, csize, rsize)
            .stroke({ width: 1, color: 0xff0000 });
        }

        for (let k = 0; k < cell.elements.length; k++) {
          const { container, strategy, originalSize, anchor } =
            cell.elements[k];
          // Size
          ...
          if (strategy == "cover!" || strategy == "stretch") {
            ...
            const scale = Math.max(
              csize / originalSize.x,
              rsize / originalSize.y
            );
